...
export default function POS() {
  ...
  const updateQty = (id: number, delta: number) => {
    ...
    setCart(prev =>
      prev.map(c => {
        ...
        return c;}).filter(c => c.quantity > 0)
    );};
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-6 relative">
      ...
      {/* Products Section */}
      <div className="flex-1 flex flex-col bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-[#f1f2f4] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
        {/* Search Header */}
        ...
        <div className="p-5 overflow-y-auto grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 custom-scrollbar">
          {filtered.map(p => (
            <div
              key={p.id}
              onClick={() => addToCart(p)}
              className="group flex flex-col bg-white/80 backdrop-blur-md border-2 border-[#f1f2f4] rounded-2xl p-4 cursor-pointer hover:border-primary-300 hover:shadow-lg hover:shadow-primary-500/5 transition-all duration-200 active:scale-[0.98] select-none relative overflow-hidden"
            >
      ...
      {/* Cart Section */}
      <div className="w-[400px] flex flex-col bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-[#f1f2f4] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden flex-shrink-0">
        {/* Customer Selector */}
        <div className="p-5 border-b border-slate-200 bg-white">
          <button className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 border-2 border-[#f1f2f4] rounded-xl hover:bg-slate-100 transition-colors group">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-white rounded-lg shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border-2 border-[#f1f2f4] flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/50">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8 text-center">
              <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mb-4 border-2 border-[#f1f2f4] border-dashed">
            ...
            <div className="divide-y divide-slate-100 p-3 space-y-2">
              {cart.map(item => (
                <div key={item.id} className="p-3.5 flex items-center gap-4 bg-white/80 backdrop-blur-md border-2 border-[#f1f2f4] rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]">
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-1 bg-slate-50 border-2 border-[#f1f2f4] rounded-lg p-0.5">
                      <button onClick={() => updateQty(item.id, -1)} className="p-1.5 rounded-md hover:bg-white hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] text-slate-500 transition-all">
        ...
        <div className="p-5 bg-white border-t border-slate-200 shadow-[0_-4px_10px_rgba(0,0,0,0.02)] z-10">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <button
              ...
              className="flex flex-col items-center justify-center gap-2 h-20 bg-white/80 backdrop-blur-md border-2 border-slate-200 rounded-2xl hover:border-blue-500 hover:bg-blue-50 text-slate-600 hover:text-blue-700 transition-all disabled:opacity-50 disabled:pointer-events-none group"
            >
            ...
            <button
              ...
              disabled={cart.length === 0}
              className="flex flex-col items-center justify-center gap-2 h-20 bg-emerald-600 border-2 border-transparent rounded-2xl hover:bg-emerald-700 text-white transition-all disabled:opacity-50 disabled:pointer-events-none shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-emerald-500/25 group"
            >
