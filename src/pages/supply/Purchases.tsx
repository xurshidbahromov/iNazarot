...
const statusConfig: Record<string, { label: string; color: string; bg: string; icon: React.ElementType; border: string}> = {"To'langan": { label:"To'langan", color:'text-emerald-700', bg:'bg-emerald-50', border:'border-emerald-200', icon: CheckCircle},"Qisman to'langan": { label:"Qisman to'langan", color:'text-amber-700', bg:'bg-amber-50', border:'border-amber-200', icon: Clock},"To'lanmagan": { label:"To'lanmagan", color:'text-red-700', bg:'bg-red-50', border:'border-red-200', icon: XCircle},
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button className="rounded-xl h-10 px-4 bg-primary-600 hover:bg-primary-700 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]" onClick={() => setIsAddModalOpen(true)}>
            ...
            return (
              <>
                ...
                <td className="whitespace-nowrap px-3 py-4 text-right text-[13px] font-medium">
                  <Button variant="outline" size="sm" className="h-8 px-2 rounded-lg text-slate-600 border-slate-200 hover:bg-slate-50  :bg-slate-800/50" onClick={() => handleViewDetails(purchase)}>
          ...
          <div className="space-y-5 p-1">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              ...
              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold border ${
                statusConfig[selectedPurchase.status]?.bg ||'bg-slate-100'} ${statusConfig[selectedPurchase.status]?.color ||'text-slate-600'} ${statusConfig[selectedPurchase.status]?.border ||'border-slate-200'}`}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border-2 border-[#f1f2f4] space-y-2.5">
