...
const typeConfig: Record<string, { label: string; color: string; bg: string }> = {
  ...
  chiqim: { label: 'Chiqim', color: 'text-red-700', bg: 'bg-red-100' },
  "ko'chirish": { label: "Ko'chirish", color: 'text-blue-700', bg: 'bg-blue-100' },
};

const statusConfig: Record<string, { label: string; icon: React.ElementType; color: string; bg: string; border: string }> = {
  draft: { label: 'Qoralama', icon: FilePlus, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border border-slate-200' },
    ...
    <div className="space-y-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-orange-100 flex items-center justify-center">
              <FileText className="w-5 h-5 text-orange-600" strokeWidth={1.6} />
            </div>
            Qoralamalar
          </h3>
          <p className="text-sm text-slate-500 mt-1">Saqlangan va ko'rib chiqilayotgan hujjatlar</p>
        </div>
        <button className="group flex items-center gap-2 px-4 py-2.5 bg-orange-600 text-white rounded-xl text-sm font-semibold hover:bg-orange-700 transition-all shadow-sm hover:shadow-orange-200 hover:shadow-md">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/30 focus:border-orange-300 transition-all placeholder:text-slate-400"
        />
      </div>
      ...
      <div className="space-y-3">
        {filtered.map(draft => {
          ...
          return (
            <div key={draft.id} className="group bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  {draft.status !== 'rejected' && (
                    <button className="px-4 py-2 bg-orange-600 text-white rounded-xl text-sm font-semibold hover:bg-orange-700 transition-all shadow-sm hover:shadow-orange-200 hover:shadow-md ml-1">
