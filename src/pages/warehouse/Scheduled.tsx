...
const typeConfig: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  chiqim: { label: 'Chiqim', color: 'text-red-700', bg: 'bg-red-50', border: 'border-red-200' },
  "ko'chirish": { label: "Ko'chirish", color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200' },
};
...
const freqConfig: Record<string, { label: string; Icon: React.ElementType }> = {
  'bir marta': { label: 'Bir marta', Icon: Hash },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-emerald-100 flex items-center justify-center">
              <Calendar className="w-5 h-5 text-emerald-600" strokeWidth={1.6} />
            </div>
        ...
        <button className="group flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-all shadow-sm hover:shadow-emerald-200 hover:shadow-md">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-300 transition-all placeholder:text-slate-400"
          />
        </div>
      </div>
        ...
        {filtered.map(item => {
          ...
          return (
            <div
              key={item.id}
              className={cn(
                'group bg-white border rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200',
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    <span className="text-xs text-slate-500 flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-lg">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  {item.status !== 'completed' && (
                    <button
                      ...
                      className={cn(
                        'p-2 rounded-xl transition-all hover:scale-110 duration-200',
                        item.status === 'active'
